...
@router.patch(
    ...
    response_model=AdminActivateUserResponseSchema,
    summary="Admin activate user",
)
async def admin_activate_user(
    user_id: int,
    db: AsyncSession = Depends(get_db),
    _: UserModel = Depends(require_roles(UserGroupEnum.ADMIN)),
) -> AdminActivateUserResponseSchema:
    ...
    await db.commit()
    return AdminActivateUserResponseSchema(
...
@router.patch(
    ...
    response_model=AdminChangeUserGroupResponseSchema,
    summary="Admin change user group",
)
async def admin_change_user_group(
    user_id: int,
    payload: AdminChangeUserGroupRequestSchema,
    db: AsyncSession = Depends(get_db),
    _: UserModel = Depends(require_roles(UserGroupEnum.ADMIN)),
) -> AdminChangeUserGroupResponseSchema:
    ...
    group_stmt = select(UserGroupModel).where(UserGroupModel.name == payload.group_name)
    ...
    await db.commit()
    return AdminChangeUserGroupResponseSchema(
        message=f"User group changed to {payload.group_name.value}.",
        user_id=user.id,
